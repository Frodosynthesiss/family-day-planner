...
2. Confirm your project matches `app.js`:
   - URL: `https://cpillnpeulshswjkdmjs.supabase.co`
   - Publishable key: `sb_publishable_tztmWNW8Ol5OlrJSbR0Hgw_rBDK6LDr`
...
1. Supabase → **password gate → Providers**
2. Enable **Email**
3. After you deploy to GitHub Pages, set:
   - **password gate → URL Configuration → Site URL** to your GitHub Pages URL
...
## Shared password mode (no individual sign-ins)

This build uses:
- A **single shared password**: `JuneR0cks!`
- A single shared Supabase scope: `household_id = 'default'`

### Supabase (new project recommended)
1. Create a new Supabase project.
2. In Supabase **SQL Editor**, run `supabase.sql`.
3. In Supabase **Project Settings → API**, copy:
   - Project URL
   - Publishable (anon) key
4. Paste them into `app.js` where indicated.

### Deploy
Host the folder as a static site (e.g., GitHub Pages). If you previously installed the PWA, you may need to refresh/clear cache after updating.
